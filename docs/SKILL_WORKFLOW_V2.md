...
Extract PPTX text/images into Deck JSON, then regenerate HTML/PPTX with the HIT visual system.
...
The importer preserves slide text and images. PPTX files generated by this project also include hidden `HIT_KIND` and `HIT_TITLE` metadata so re-imported pages keep their intended layout kind and title.

### 3.1 MarkItDown Source Conversion

When the user provides mixed source files such as PPTX, PDF, DOCX, XLSX, or web-captured documents, use MarkItDown to normalize the file into Markdown before layout:

```bash
npm run deck -- --source old-deck.pptx --template academic-tech-dark --outDir generated/from-source
```

Decision rule:

- Use `--pptx` for PowerPoint files when image extraction and generated-deck metadata preservation matter.
- Use `--source` / `--file` for PPTX, PDF, DOCX, or other source files when broad text/table extraction matters more than preserving the original slide geometry.
- Use `--pptx old-deck.pptx --useMarkitdown` when a PPTX should explicitly go through Markdown normalization.
