...
Phase 2 now has a working baseline:

- importing existing PPTX text/images into Deck JSON;
- preserving page kinds for PPTX files generated by this project through hidden metadata;
- reflowing imported Deck JSON back into HTML and editable PPTX.
...
- `npm run import:pptx` must extract an editable Deck JSON baseline from generated PPTX.
- `npm run smoke` must pass before release.
